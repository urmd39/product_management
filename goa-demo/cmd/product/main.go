...
func main() {
	...
	{
		logger = log.New(os.Stderr, "[productapi] ", log.Ltime)
	}
	...
	{
		productSvc = productapi.NewProduct(logger)
	}
	...
	case "localhost":
		{
			addr := "http://localhost:8000/product"
		...
		{
			addr := "grpc://localhost:8080/product"
			u, err := url.Parse(addr)
			if err != nil {
				fmt.Fprintf(os.Stderr, "invalid URL %#v: %s\n", addr, err)
				os.Exit(1)
